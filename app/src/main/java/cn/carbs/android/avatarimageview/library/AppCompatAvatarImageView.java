...
public class AppCompatAvatarImageView extends AppCompatImageView {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, android.carbs.cn.library.R.styleable.AvatarImageView);
        if (a == null) {
            return;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_TextSizeRatio) {
                mTextSizeRatio = a.getFloat(attr, DEFAULT_TEXT_SIZE_RATIO);
            }else if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_TextMaskRatio){
                mTextMaskRatio = a.getFloat(attr, DEFAULT_TEXT_MASK_RATIO);
            }else if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_BoarderWidth) {
                mBoarderWidth = a.getDimensionPixelSize(attr, DEFAULT_BOARDER_WIDTH);
            }else if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_BoarderColor) {
                mBoarderColor = a.getColor(attr, DEFAULT_BOARDER_COLOR);
            }else if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_TextColor) {
                mTextColor = a.getColor(attr, DEFAULT_TEXT_COLOR);
            }else if (attr == android.carbs.cn.library.R.styleable.AvatarImageView_aiv_ShowBoarder){
